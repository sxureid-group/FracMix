...
def extract_cnn_feature(model, inputs):
    ...
    outputs, topk_weight= model(inputs)
    # outputs = model(inputs)[0]
    # out_rb = model(inputs)[1]
    # print(outputs.shape)
    outputs = outputs.data.cpu()
    return outputs, topk_weight
...
def extract_features(model, data_loader, print_freq=50):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs, topk_weight = extract_cnn_feature(model, imgs)
    ...
    return features, topk_weight, labels
...
class Evaluator(object):
    ...
    def evaluate(self, data_loader, query, gallery, cmc_flag=False, rerank=False):
        features, topk_weight, _ = extract_features(self.model, data_loader)

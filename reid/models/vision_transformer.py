...
class SEModule(nn.Module):
    ...
    def forward(self, x):
        ...
        y = x
        # Excitation: Learn to weight the channels
        y = self.fc(y).view(b, c)
        # Scale the input with the learned weight
        weighted_x = x * y
        return weighted_x
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, conv_stem=False):
        super().__init__()
        ...
        self.senet = nn.Sequential(
            nn.Linear(embed_dim * num_patches, num_patches),# # 将输入特征映射到 num_patches 维度
            nn.ReLU(),
    ...
    def forward_features(self, x):
        ...
        out = torch.reshape(x,(B, self.patch_embed.num_patches * self.embed_dim))
        out = self.senet(out)
        out_softmax = out.softmax(dim=1)

        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks  64 1 768
        ...
        return [(x[:, 0]) for x in layer_wise_tokens], out_softmax

    def forward(self, x):
        list_out, topk_weight = self.forward_features(x)
        x1 = list_out[-1]
        x = [self.fc(x) for x in list_out]

        block_number1 = random.randint(0, 5)  # blocks sampling
        block_number2 = random.randint(6, len(self.blocks) - 1)  # blocks sampling
        x_random_block1 = x[block_number1]
        x_random_block2 = x[block_number2]
        alpha = 1.0
        lam = numpy.random.beta(alpha, alpha)
        x_random_block = lam * x_random_block1 + (1 - lam) * x_random_block2

        low_f_0, low_f_1, low_f_2 = self.se1(x[0]), self.se1(x[1]), self.se1(x[2])
        low_f_3, low_f_4, low_f_5 = self.se2(x[3]), self.se2(x[4]), self.se2(x[5])
        high_f_6, high_f_7, high_f_8 = self.se3(x[6]), self.se3(x[7]), self.se3(x[8])
        high_f_9, high_f_10, high_f_11 = self.se4(x[9]), self.se4(x[10]), self.se4(x[11])
        ...
        return x1, x_random_block, out, topk_weight

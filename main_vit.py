...
def get_train_loader(args, dataset, height, width, batch_size, workers,
    ...
    train_set = sorted(dataset.mix_dataset) if trainset is None else sorted(trainset) # 对集合进行排序 ???
    rmgs_flag = num_instances > 0
    if rmgs_flag:
        # sampler = RandomMultipleGallerySampler(train_set, num_instances)
        sampler = RandomMultipleGallerySamplerNoCam(train_set, num_instances)   # 训练集随机采样
    else:
        sampler = None
...
def main_worker(args):
    ...
    sys.stdout = Logger(osp.join(args.logs_dir, 'log.txt'))    # 日志文件路径
    print("==========\nArgs:{}\n==========".format(args))
    ...
    for dataset_i in range(len(datasets_src)):
        ...
        source_features, _, _ = extract_features(model, sour_cluster_loader, print_freq=50)
        sour_fea_dict = collections.defaultdict(list)

        for f, pid, _ in sorted(dataset_source.mix_dataset):  # f: filename pid:1(person id )
            sour_fea_dict[pid].append(source_features[f].unsqueeze(0))  # 2048
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default='/home/xiehuidi/datasets')
    parser.add_argument('--logs-dir', type=str, metavar='PATH',
                        default=osp.join(working_dir, './logs/1'))
